...
func initAtofOnce() {
	...
	for i := range benchmarksRandomBits {
		...
		benchmarksRandomBits[i] = FormatFloat(buf, x, 'g', -1, 64)
	}

	for i := range benchmarksRandomNormal {
		x := rand.NormFloat64()
		benchmarksRandomNormal[i] = FormatFloat(buf, x, 'g', -1, 64)
	}
}
...
func TestAtof(t *testing.T) {
	...
	for i := 0; i < len(atoftests); i++ {
		...
		outs := FormatFloat(buf, out, 'g', -1, 64)
		...
		if float64(float32(out)) == out {
			...
			outs := FormatFloat(buf, float64(out32), 'g', -1, 32)
			if outs != test.out || err != test.err {
...
func TestRoundTrip32(t *testing.T) {
	...
	for i := uint32(0); i < 0xff<<23; i += step {
		...
		s := FormatFloat(buf, float64(f), 'g', -1, 32)
